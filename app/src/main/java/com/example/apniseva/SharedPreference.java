...
public class SharedPreference {
    ...
    /*public void editFavorite(Context context, int beanSampllist, String textprice, String quanity){
        ...
        if(favorites!=null){
            ...
            String ranjeet= favorites.get(beanSampllist).getPrice();
            Log.d("RanjeetShared",ranjeet);

            favorites.get(beanSampllist).setPrice(textprice);

            Log.d("RanjeetShared",favorites.get(beanSampllist).getPrice());

            favorites.get(beanSampllist).setPrice(quanity);
            storeFavorites(context, favorites);
        }
    ...
    public double getCartAmount(Context context){
        ...
        for (int j = 0; j < itms.size(); j++) {
            ...
            sum = sum + (Double.parseDouble(cartIt.getPrice()));
